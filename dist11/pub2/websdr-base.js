...
var lo=-2.95,hi=-0.19;   // edges of passband, in kHz w.r.t. the carrier
var cwlo=-0.95, cwhi=-0.55;   // PA0SIM memories for CW passband settings
var cwnlo=-0.78, cwnhi=-0.72;   
var lsblo=-2.95, lsbhi=-0.19;    // PA0SIM memories for LSB passband settings
var lsbnlo=-2.2, lsbnhi=-0.30;    
var usblo=0.19, usbhi=2.95;      // PA0SIM memories for USB passband settings
var usbnlo=0.30, usbnhi=2.2;      
var amlo=-4.50, amhi=4.50;    // PA0SIM memories for AM passband settings
var amnlo=-2.50, amnhi=2.50;
var fmlo=-4.50, fmhi=4.50;    // PA0SIM memories for FM passband settings
var fmnlo=-3.25, fmnhi=3.25;    
...
function set_mode(m)      // ...with appropriate filter
{
   switch (m.toUpperCase()) {
      case "USBN": setmf("usb", usbnlo, usbnhi); break;
      case "USB": setmf("usb", usblo,usbhi); break;
      case "LSBN": setmf("lsb", lsbnlo, lsbnhi); break;
      case "LSB": setmf("lsb", lsblo, lsbhi); break;
      case "AMN": setmf("am", amnlo, amnhi); break;
      case "AM":  setmf("am", amlo, amhi); break;
      case "CWN":  setmf("cw", cwnlo, cwnhi); break;
      case "CW":  setmf("cw", cwlo, cwhi); break;
      case "FMN": setmf("fm", fmnlo, fmnhi); break;
      case "FM":  setmf("fm", fmlo, fmhi); break;
   }
}
...
function setfreqtune(s)
{
   ...
   if (!param[1]) return;
   if (param[2]) set_mode(param[2]);
   if (param[2].toUpperCase()!="CW") setfreqif(param[1]);		// PA0SIM bug correction
   else {
   	setfreqifCW(param[1]);
   	}
}
...
function mem_recall(i)
{
   ...
   updbw();
   var corrCW=0;
   if (mode=="CW") corrCW=-(lo+hi)/2;							// PA0SIM bug correction
   setfreq(memories[i].freq+corrCW);
   setwaterfall(band,memories[i].freq+corrCW);
   showdx(memories[i].band);										// PA0SIM bug correction
   if (mode=="CW") {cwlo=lo; cwhi=hi;}			// PA0SIM
	if (mode=="LSB") {lsblo=lo; lsbhi=hi;}		// PA0SIM
	if (mode=="USB") {usblo=lo; usbhi=hi;}		// PA0SIM
	if (mode=="AM") {amlo=lo; amhi=hi;}			// PA0SIM
}
...
function mem_store(i)
{
   ...
   if (mode=="CW") corrCW=(lo+hi)/2;							// PA0SIM bug correction
   try { l=memories[i].label;} catch(e){ l=''; };
   memories[i]={freq:freq+corrCW, nomfreq:nomf, band:band, mode:mode, lo:lo, hi:hi, label:l };
   mem_show();
   showdx(memories[i].band);
   try { localStorage.setItem('memories',JSON.stringify(memories)); } catch (e) {};
}
...
{
   // returns true if default SSB mode for this band should be LSB --- does not work!
   var e=bi[b];
...
function setband(b)
{
   ...
   if (!islsbband(b) && (hi<0)&&(mode!="CW")) { var tmp=hi; hi=-lo; lo=-tmp; mode="USB"; }  //PA0SIM bug correction
   if (islsbband(b) && (hi>0)&&(mode!="CW")) { var tmp=hi; hi=-lo; lo=-tmp; mode="LSB"; }   //PA0SIM bug correction
   // PA0SIM next lines are not correct
   //if (islsbband(band)!=islsbband(b)) {
      // if needed, exchange LSB/USB 
   //   var tmp=hi;
   //   hi=-lo;
   ...
   if (view==Views.allbands || view==Views.othersslow) {
      ...
      if (!hidedx) {
         ...
         dxs=[]; document.getElementById('blackbar0').innerHTML=""; 
         fetchdx(b);
         showdx(b);					// PA0SIM bug correction
       }
   }
   setwaterfall(b,e.vfo);
   centerfreq = e.effcenterfreq;
